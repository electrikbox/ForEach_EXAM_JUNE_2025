  ...
  <div v-if="visible" 
    ...
    <div 
      class="bg-white rounded-none w-full h-full overflow-y-auto shadow-none relative
             sm:rounded-lg sm:max-w-md sm:h-auto sm:shadow-2xl" @click.stop
    >
      <button 
        class="absolute top-4 right-4 text-gray-600 hover:text-gray-900 text-3xl leading-none p-1 z-10" 
        @click="fermerModal"
      >
        &times;
      </button>
      ...
      <img 
        :src="cocktail?.imgUrl ? `${cocktail.imgUrl}&w=500&fit=crop` : 'https://via.placeholder.com/500x256?text=Cocktail'"
        :alt="cocktail?.nomCocktail"
        class="w-full h-64 object-cover"
      />

      <div class="p-6">
        <h1 class="text-3xl font-bold text-gray-900 mb-4">{{ cocktail?.nomCocktail }}</h1>
        <p class="text-base text-gray-700 leading-relaxed mb-3">{{ cocktail?.descriptionCocktail }}</p>
        
        <div 
          v-if="cocktail?.categorie" 
          class="inline-block bg-gray-200 text-gray-700 text-sm font-medium px-3 py-1 rounded-full mb-4"
        >
